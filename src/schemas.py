...
class PaginatedDataResponse(BaseModel, Generic[T]):
    total_count: int = Field(
        ..., description="Total count of data", json_schema_extra={"examples": [100]}
    )
    total_pages: int = Field(..., description="Total pages", json_schema_extra={"examples": [10]})
    current_page: int = Field(..., description="Current page", json_schema_extra={"examples": [1]})
    data: list[T]
